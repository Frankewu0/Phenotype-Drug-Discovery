...
class Trainer(object):
    ...
    def test(self, dataloader="test",output = 'No'):
        test_loss = 0
        y_label, y_pred = [], []
        if dataloader == "test":
...
class Pred(object):
    def __init__(self,model,device, n_class=2):
        self.model = model
        self.device = device
        self.n_class = n_class

    def test_pred(self, dataloader):
        test_loss = 0
        y_pred = []

        data_loader = dataloader
        num_batches = len(data_loader)
        trained_weights = torch.load("result/demo/best_model_epoch_1.pth")
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(data_loader):
                ...
                if self.n_class == 1:
                    n = torch.sigmoid(score)
                else:
                    n = torch.softmax(score, dim=1)
                y_pred = y_pred + n.to("cpu").tolist()
