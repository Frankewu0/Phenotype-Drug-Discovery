...
from tansformer_msa import MSA_Transformer
from torch.nn import TransformerEncoder, TransformerEncoderLayer
from model.mpcat import (Decoder,Encoder,Predictor,PositionwiseFeedforward,
DecoderLayer,SelfAttention,Trainer,Tester)
...
class DrugBAN(nn.Module):
    def __init__(self, **config):
        ...
        # self.protein_extractor = MSA_Transformer(d_model,protein_emb_dim, hid_dim,nhead=8, num_layers=6, num_classes=2)
        self.protein_extractor = ProteinTransformer(protein_emb_dim, d_model,nhead=8, num_layers=6)
        ...
        self.mprocat = Decoder(atom_dim, hid_dim, n_layers, n_heads, pf_dim, DecoderLayer, SelfAttention, PositionwiseFeedforward, dropout,device)
...
class ProteinTransformer(nn.Module):
    def __init__(self, embedding_dim, d_model, nhead, num_layers):
        super(ProteinTransformer, self).__init__()
        
        self.embedding = nn.Embedding(26, embedding_dim, padding_idx=0)  # 可以修改 embedding_dim
        ...
        self.fc = nn.Linear(d_model, d_model)  # 可以修改 d_model

    def forward(self, src):
        # 可以修改你的批处理大小，以适应你的内存容量
