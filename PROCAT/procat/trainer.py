...
from .model.models import binary_cross_entropy, cross_entropy_logits, entropy_logits, RandomLayer
from .model.layer import ReverseLayerF
...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader, opt_da=None, discriminator=None,
        ...
        self.optim = optim
        self.device = device
        self.epochs = config["MODEL"]["MAX_EPOCH"]
        self.current_epoch = 0
        self.train_dataloader = train_dataloader
        self.val_dataloader = val_dataloader
        self.test_dataloader = test_dataloader
        self.is_da = config["MODEL"]["USE"]
        self.alpha = alpha
        self.n_class = config["MODEL"]["BINARY"]
        if opt_da:
            self.optim_da = opt_da
        self.da_init_epoch = config["MODEL"]["INIT_EPOCH"]
        self.init_lamb_da = config["MODEL"]["LAMB_DA"]
        self.batch_size = config["MODEL"]["BATCH_SIZE"]
        self.use_da_entropy = config["MODEL"]["USE_ENTROPY"]
        self.nb_training = len(self.train_dataloader)
        self.step = 0
        self.experiment = experiment

        if config["model_task"] == "predict":
            state_dict = torch.load(os.path.join(base_dir,"../model_weights/checkpoints/best_model.pth"), map_location=torch.device('cuda:0'))
        ...
        self.pred_output_dir = config["RESULT"]["PRED_OUTPUT_DIR"]
        ...
        self.original_random = config["MODEL"]["ORIGINAL_RANDOM"]
    ...
    def train_da_epoch(self):
        ...
        if self.current_epoch >= self.da_init_epoch:
            ...
            epoch_lamb_da = 1
            if self.experiment:
                self.experiment.log_metric("MODEL loss lambda", epoch_lamb_da, epoch=self.current_epoch)
        num_batches = len(self.train_dataloader)
    ...
    def test(self, dataloader="test"):
        ...
        pred['pred'] = round(pred['pred'],3)
        pred.to_csv(os.path.join(self.output_dir,"result.csv"),index=0)
                

        if len(np.unique(y_label)) < 2:
            print("Warning: only one class in test labels, skipping ROC AUC calculation.")
            auroc = float("nan")  # 或者 auroc = 0.0
        else:
